...
4. **List existing design docs for this project:**
   ```bash
   ls -t ~/.orch/projects/$SLUG/*-design-*.md 2>/dev/null
   ```
...
```bash
grep -li "<keyword1>\|<keyword2>\|<keyword3>" ~/.orch/projects/$SLUG/*-design-*.md 2>/dev/null
```
...
This enables cross-team discovery — multiple users exploring the same project will see each other's design docs in `~/.orch/projects/`.
...
```bash
SKETCH_FILE="/tmp/orch-sketch-$(date +%s).html"
```
...
$B screenshot /tmp/orch-sketch.png
```
...
The screenshot file at `/tmp/orch-sketch.png` can be referenced by downstream skills
...
```bash
PRIOR=$(ls -t ~/.orch/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
```
If `$PRIOR` exists, the new doc gets a `Supersedes:` field referencing it. This creates a revision chain — you can trace how a design evolved across office hours sessions.

Write to `~/.orch/projects/{slug}/{user}-{branch}-design-{datetime}.md`:
...
echo '{"skill":"office-hours","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.orch/analytics/spec-review.jsonl 2>/dev/null || true
```
...
- If yes → run `open https://ycombinator.com/apply?ref=orch` and say: "Bring this design doc to your YC interview. It's better than most pitch decks."
...
>
> **ycombinator.com/apply?ref=orch**
...
The design doc at `~/.orch/projects/` is automatically discoverable by downstream skills — they will read it during their pre-review system audit.
