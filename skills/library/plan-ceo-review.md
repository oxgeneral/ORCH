...
**Design doc check:**
```bash
SLUG=$(~/.claude/skills/orch/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
DESIGN=$(ls -t ~/.orch/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t ~/.orch/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
...
```bash
HANDOFF=$(ls -t ~/.orch/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null | head -1)
...
```bash
mkdir -p ~/.orch/projects/$SLUG
USER=$(whoami)
DATETIME=$(date +%Y%m%d-%H%M%S)
```
Write to `~/.orch/projects/$SLUG/$USER-$BRANCH-ceo-handoff-$DATETIME.md`:
```markdown
...
```bash
source <(~/.claude/skills/orch/bin/orch-slug 2>/dev/null) && mkdir -p ~/.orch/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p ~/.orch/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv ~/.orch/projects/$SLUG/ceo-plans/{old-plan}.md ~/.orch/projects/$SLUG/ceo-plans/archive/
```

Write to `~/.orch/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
3. Append metrics:
```bash
mkdir -p ~/.orch/analytics
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.orch/analytics/spec-review.jsonl 2>/dev/null || true
```
...
```
NOTE: These represent human-team implementation hours. With CC + orch,
...
* **Effort estimate:** S/M/L/XL (human team) → with CC+orch: S→S, M→S, L→M, XL→L
* **Priority:** P1/P2/P3
...
rm -f ~/.orch/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null || true
```
...
`~/.orch/` (user config directory, not project files). The skill preamble
already writes to `~/.orch/sessions/` and `~/.orch/analytics/` — this is
the same pattern. The review dashboard depends on this data. Skipping this
command breaks the review readiness dashboard in /ship.

```bash
~/.claude/skills/orch/bin/orch-review-log '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","scope_proposed":N,"scope_accepted":N,"scope_deferred":N,"commit":"COMMIT"}'
```
...
```bash
~/.claude/skills/orch/bin/orch-review-read
```
...
**Review tiers:**
- **Eng Review (required by default):** The only review that gates shipping. Covers architecture, code quality, tests, performance. Can be disabled globally with \`orch-config set skip_eng_review true\` (the "don't bother me" setting).
...
- Search the plan file for a \`## ORCH REVIEW REPORT\` section **anywhere** in the file
  (not just at the end — content may have been added after it).
- If found, **replace it** entirely using the Edit tool. Match from \`## ORCH REVIEW REPORT\`
...
- **B)** Keep in `~/.orch/projects/` only (local, personal reference)
- **C)** Skip
