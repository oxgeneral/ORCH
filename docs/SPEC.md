...
КАЖДЫЙ ТИКТ (poll_interval_ms):
  ...
  2. Dispatch
     ...
     - Отсортировать по приоритету → дате создания
...
type OrchestratorEvent =
  ...
  | { type: 'agent:output'; runId: string; data: string }
  | { type: 'agent:file_changed'; runId: string; path: string }
  | { type: 'agent:completed'; runId: string; success: boolean }
  | { type: 'agent:error'; runId: string; error: string }
...
- [ ] Инициализация проекта (TypeScript, Commander.js)
- [ ] Файловое хранилище (`.orchestry/`, YAML tasks/agents)
- [ ] Task CRUD (add, list, show, edit, cancel)
- [ ] Agent CRUD (add, list, remove)
- [ ] Claude Code adapter (базовый)
- [ ] Shell adapter
- [ ] Оркестратор: dispatch одной задачи одному агенту
- [ ] CLI-команда `orchestry run <task-id>`
- [ ] Базовое логирование
- [ ] `orchestry status` — текстовый вывод

### Phase 2: Multi-Agent & TUI (2 недели)

- [ ] Параллельный запуск нескольких агентов
- [ ] Retry с exponential backoff
- [ ] Stall detection
- [ ] Режим демона (`orchestry run --watch`)
- [ ] TUI dashboard (Ink)
- [ ] Live-логи (`orchestry logs --follow`)
- [ ] Workspace modes (shared, worktree, isolated)

### Phase 3: Smart Orchestration (2 недели)

- [ ] Автоматическое назначение задач агентам (по роли/меткам)
- [ ] Зависимости между задачами (`depends_on`)
- [ ] Reconciliation (проверка состояния при рестарте)
- [ ] Шаблоны промптов (Liquid engine)
- [ ] Codex adapter
- [ ] Cursor adapter
- [ ] Статистика и метрики (tokens, runtime)

### Phase 4: Governance & Polish (2 недели)

- [ ] Review mode (задача → review → approve/reject)
- [ ] Proof of work (git diff, test results, summary)
- [ ] Export/import задач (Markdown, JSON)
- [ ] Интеграция с GitHub Issues (опционально)
- [ ] Плагинная система адаптеров
