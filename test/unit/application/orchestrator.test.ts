...
describe('Orchestrator', () => {
  ...
  describe('dispatch ordering (priority + goals)', () => {
    async function dispatchWithSingleSlot(tasks: Task[]) {
      const registry = new AdapterRegistry();
      registry.register(createMockAdapter([
        { type: 'done', timestamp: new Date().toISOString(), data: { result: 'done' } },
      ]));
      ...
      await orchestrator.startWatch();
      await new Promise((r) => setTimeout(r, 100));
      return (deps.taskStore.save as ReturnType<typeof vi.fn>).mock.calls;
    }

    function wasDispatched(saves: [Task][], id: string): boolean {
      return saves.some(([t]) => t.id === id && t.status === 'in_progress');
    }

    it('dispatches higher-priority tasks first (P1 before P3)', async () => {
      const saves = await dispatchWithSingleSlot([
        makeTask({ id: 'tsk_p3', priority: 3, updated_at: '2025-01-02T00:00:00Z' }),
        makeTask({ id: 'tsk_p1', priority: 1, updated_at: '2025-01-01T00:00:00Z' }),
      ]);
      expect(wasDispatched(saves, 'tsk_p1')).toBe(true);
      expect(wasDispatched(saves, 'tsk_p3')).toBe(false);
    });

    it('dispatches goal-linked tasks before unlinked at same priority', async () => {
      const saves = await dispatchWithSingleSlot([
        makeTask({ id: 'tsk_nogoal', priority: 2, updated_at: '2025-01-02T00:00:00Z' }),
        makeTask({ id: 'tsk_goal', priority: 2, goalId: 'goal_abc', updated_at: '2025-01-01T00:00:00Z' }),
      ]);
      expect(wasDispatched(saves, 'tsk_goal')).toBe(true);
      expect(wasDispatched(saves, 'tsk_nogoal')).toBe(false);
    });

    it('priority trumps goal-linking (P1 no goal beats P2 with goal)', async () => {
      const saves = await dispatchWithSingleSlot([
        makeTask({ id: 'tsk_p2g', priority: 2, goalId: 'goal_abc' }),
        makeTask({ id: 'tsk_p1', priority: 1 }),
      ]);
      expect(wasDispatched(saves, 'tsk_p1')).toBe(true);
      expect(wasDispatched(saves, 'tsk_p2g')).toBe(false);
    });

    it('recency breaks ties within same priority and goal status', async () => {
      const saves = await dispatchWithSingleSlot([
        makeTask({ id: 'tsk_old', priority: 2, updated_at: '2025-01-01T00:00:00Z' }),
        makeTask({ id: 'tsk_new', priority: 2, updated_at: '2025-01-02T00:00:00Z' }),
      ]);
      expect(wasDispatched(saves, 'tsk_new')).toBe(true);
    });
  });
